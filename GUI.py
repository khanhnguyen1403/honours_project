...
class ScrollableGrid(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        # Inner frame where the actual content lives
...
class App:
    def __init__(self, root, appliances):
        ...
        # Periodic updates
        self.root.after(500, self.generate_new_values)  # Every 0.5s: simulate data
        self.root.after(500, self.update_graphs)        # Every 0.5s: update graphs
    ...
    def generate_new_values(self):
        ...
        self.root.after(500, self.generate_new_values)
    ...
    def auto_restart(self, appliance):
        ...
        if self.restart_attempts[appliance] < 1:
            ...
            self.log_event(f"{appliance}: Auto recovery after reverse current event.")
        else:
            # If there have been 2 attempts already, lock auto restart
            self.log_event(f"{appliance}: Auto restart locked. Please manually turn on the appliance.")
