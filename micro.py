...
def send_sensor_data(voltage, current, power, cycle_id):
    send_at_command(f'+TEST=TXLRSTR,"V{cycle_id}:{voltage:.2f}"')
    sleep_ms(100)
    
    send_at_command(f'+TEST=TXLRSTR,"I{cycle_id}:{current:.2f}"')
    sleep_ms(100)
    
    send_at_command(f'+TEST=TXLRSTR,"P{cycle_id}:{power:.2f}"')
    sleep_ms(100)
...
#     sleep(0.05)  # Delay 
    ...
    send_sensor_data(voltage, current, power, cycle_id)
    cycle_id += 1
