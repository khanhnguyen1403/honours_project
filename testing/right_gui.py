...
class Right_GUI:
    ...
    def create_dynamic_settings(self):
        ...
        # Log restoration if values were loaded
        if current_appliance and any(setting[1] for setting in settings):
            self.log_events(f"Settings restored for {current_appliance.name}")
    ...
    def save_settings(self):
        ...
        if self.upper_gui:
            current_appliance = self.upper_gui.get_current_appliance()
            if current_appliance is not None:  # Don't update if "All" is selected
